...
/**
 * update_sidebars.php
 * Replaces <aside class="app-sidebar">...</aside> in all admin-area PHP files
 * with a role-aware sidebar:
 *   - admin  → User Approvals + Add Account only
 *   - others → full procurement nav (no admin-only items)
 */
...
function updateSidebarsRecursively($dir, $sidebar_replacement) {
    ...
    foreach ($iterator as $file) {
        if ($file->isFile() && $file->getExtension() === 'php') {
            ...
            // Skip non-admin-area directories
